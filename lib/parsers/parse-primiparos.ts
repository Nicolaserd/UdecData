...
interface PrimiparosRow {
  "NOMBRE PROGRAMA": string;
  MUNICIPIO: string;
  "AÑO": string;
  SEMESTRE: string;
  [key: string]: string;
...
export function parsePrimiparos(
  csvText: string
): { rows: NormalizedStudentRow[]; warnings: string[] } {
  const warnings: string[] = [];
  const clean = csvText.replace(/^\uFEFF/, "");

  const parsed = Papa.parse<PrimiparosRow>(clean, {
    delimiter: ";",
    header: true,
    skipEmptyLines: true,
    quoteChar: '"',
  ...
  if (parsed.errors.length > 0) {
    warnings.push(`Primíparos: ${parsed.errors.length} errores de parseo`);
  }

  const rows: NormalizedStudentRow[] = [];

  for (const row of parsed.data) {
    const programa = row["NOMBRE PROGRAMA"];
    ...
    if (!progMatched) {
      warnings.push(`Primíparos: programa no reconocido "${programa}"`);
    }

    const { unidadRegional, matched: muniMatched } = mapMunicipio(municipio);
    if (!muniMatched) {
      warnings.push(`Primíparos: municipio no reconocido "${municipio}"`);
    }
