...
interface GraduadosRow {
  PROGRAMA: string;
  "MUNICIPIO PROGRAMA": string;
  "AÑO": string;
  SEMESTRE: string;
  [key: string]: string;
}

export function parseGraduados(
  csvText: string
): { rows: NormalizedStudentRow[]; warnings: string[] } {
  const warnings: string[] = [];
  const clean = csvText.replace(/^\uFEFF/, "");

  const parsed = Papa.parse<GraduadosRow>(clean, {
    delimiter: ";",
    header: true,
    skipEmptyLines: true,
    quoteChar: '"',
  ...
  if (parsed.errors.length > 0) {
    warnings.push(`Graduados: ${parsed.errors.length} errores de parseo`);
  }
  ...
  for (const row of parsed.data) {
    ...
    const municipio = row["MUNICIPIO PROGRAMA"];
    ...
    if (!progMatched) {
      warnings.push(`Graduados: programa no reconocido "${programa}"`);
    }

    const { unidadRegional, matched: muniMatched } = mapMunicipio(municipio);
    if (!muniMatched) {
      warnings.push(`Graduados: municipio no reconocido "${municipio}"`);
    }
