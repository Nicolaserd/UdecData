...
export async function GET() {
  try {
    const data = await prisma.planMejoramientoDocente.findMany({
      orderBy: [
        { anio: "asc" },
        { encuentro: "asc" },
        { unidad_regional: "asc" },
    ...
    ws["!cols"] = [
      ...
      { wch: 25 }, { wch: 60 }, { wch: 10 }, { wch: 35 },
      { wch: 20 }, { wch: 35 }, { wch: 8 }, { wch: 20 },
      { wch: 10 }, { wch: 15 },
    ];
    XLSX.utils.book_append_sheet(wb, ws, "PM Docentes");
    ...
    return new NextResponse(new Uint8Array(buffer), {
      ...
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="PM_DOCENTES_ENCUENTROS_DIALOGICOS.xlsx"',
      },
    });
  } catch (error) {
